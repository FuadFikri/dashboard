 <div class="menu_section">
                <h3>General</h3>
                <ul class="nav side-menu">
                    @if($user = Auth::user())
                    <li><a href="{{ url('/data') }}"><i class="fa fa-database"></i> Data</a></li>
                    <li><a href="{{ url('/trash') }}"><i class="fa fa-trash"></i> Trash</a></li>
                    <li><a><i class="fa fa-book"></i> Documentation <span class="fa fa-chevron-down"></span></a>
                      <ul class="nav child_menu">
                        <li><a href="{{ url('/Documentation#content1') }}">Content 1</a></li>
                        <li><a href="{{ url('/Documentation#content2') }}">Content 2</a></li>
                        <li><a href="{{ url('/Documentation#content3') }}">Content 3</a></li>
                      </ul>
                    </li>
                    @else
                    <li><a><i class="fa fa-book"></i> Documentation <span class="fa fa-chevron-down"></span></a>
                      <ul class="nav child_menu">
                        <li><a href="{{ url('/Documentation#content1') }}">Content 1</a></li>
                        <li><a href="{{ url('/Documentation#content2') }}">Content 2</a></li>
                        <li><a href="{{ url('/Documentation#content3') }}">Content 3</a></li>
                      </ul>
                    </li>
                    @endif
                </ul>
              </div>
              <div class="menu_section">
            </div>
